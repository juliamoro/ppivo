...
public class View {
	...
	private void addItems(){
        ...
        ((Button)addItemWindow.getDialogPane().lookupButton(addItemWindow.getButtonTypes().get(0))).setOnAction(ae->{
            if(petNameField.getText().matches(".+")
                    && birthdayField.getValue() != null
                    && lastAppointmentField.getValue() != null
                    && surnameField.getText().matches(".+")
                    && nameField.getText().matches(".+")
                    && patronymField.getText().matches(".+")
                    && diagnosisField.getText().matches(".+")){ // фун проверкм точка это любой символ + 1 и больше
                controller.addPet(
